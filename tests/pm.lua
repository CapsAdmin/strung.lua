...
function f1(s, p)
	p = strung.gsub(p, "%%([0-9])", function(s)
		return "%" .. (s + 1)
	end)
	p = strung.gsub(p, "^(^?)", "%1()", 1)
	p = strung.gsub(p, "($?)$", "()%1", 1)

	if _G.LOL then print(p) end
...
print("+")
_G.LOL = true
assert(f1("alo alx 123 b\0o b\0o", "(..*) %1", true) == "b\0o b\0o")
_G.LOL = nil
...
assert(strung.gsub("abc=xyz", "(%w*)(%p)(%w+)", "%3%2%1-%0") == "xyz=abc-abc=xyz")
...
print("+")
assert(
	strung.gsub("um (dois) tres (quatro)", "(%(%w+%))", string.upper) == "um (DOIS) tres (QUATRO)"
)
...
function f(a, b)
	return strung.gsub(a, ".", b)
end

assert(
	strung.gsub("trocar tudo em |teste|b| ? |beleza|al|", "|([^|]*)|([^|]*)|", f) == "trocar tudo em bbbbb ? alalalalalal"
)

local function dostring(s)
	return loadstring(s)() or ""
end

assert(
	strung.gsub("alo $a=1$ novamente $return a$", "$([^$]*)%$", dostring) == "alo  novamente 1"
)
x = strung.gsub(
	"$x=strung.gsub('alo', '.', string.upper)$ assim vai para $return x$",
	"$([^$]*)%$",
	dostring
)
...
assert(
	strung.gsub("x and x and x", "x", function()
			t.n = t.n + 1
			return t[t.n]
...
strung.gsub("first second word", "%w%w*", function(w)
	t.n = t.n + 1
	t[t.n] = w
end)
...
-- deep nest of gsubs
function rev(s)
	return strung.gsub(s, "(.)(.+)", function(c, s1)
		return rev(s1) .. c
	end)
end
